...
| **Daily Production** | Confirm completed prep, record actual batch output, retain finished stock, capture planned or walk-up venue pickups, allocate separate production/dishwasher labor, and finalize one daily closeout. |
...
4. **Daily production closeout** (`/production`) — completed prep lots appear as confirmable defaults. Production freezes actual output, ingredient detail, production person-minutes, and the wage-only production rate. Standalone batches and estimated opening stock can also be entered. Venue transfers are separate records and consume finished stock on the transfer date, so retained stock and later pickups are charged exactly once. Dishwasher person-minutes are entered once per day, previewed, allocated by production person-minutes with penny reconciliation, and frozen at manager closeout. Waste, returns, borrowed ingredients, and venue-supplied exclusions remain explicit audit records.
