...
import { assertOrderVenue } from "@/lib/prep-access";
...
async function editable(
  ...
  assertOrderVenue(user, order.destinationVenueId);
...
export async function addPrepLine(f: FormData) {
  ...
  await serializable(async (tx) => {
    const order = await editable(tx, id, user);
...
export async function updatePrepLine(f: FormData) {
  ...
  await serializable(async (tx) => {
    ...
    await editable(tx, line.prepOrderId, user);
...
export async function generatePacket(f: FormData) {
  const user = await requirePrepUser("MANAGER");
